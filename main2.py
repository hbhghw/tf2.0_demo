...
def build_model():  # ResNet50
    ...
    x = layers.Dense(units=1,activation='sigmoid')(x)  # [batch_size,1]
...
def compute_acc(y_true, y_pred):
    return tf.reduce_mean(tf.cast(tf.equal(y_true, tf.where(y_pred > 0.5, 1., 0.)), tf.float32))
    ...
    for data, labels in train_dataset:
        ...
        if step % 2 == 0:
            ...
            for batch_val_data, batch_val_labels in val_dataset:
                ...
                correct += np.sum(np.where(batch_val_labels > 0.5, 1, 0) == np.where(pred > 0.5, 1, 0))
